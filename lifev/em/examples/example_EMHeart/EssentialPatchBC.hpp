...
public:
	...
	//std::cout << "This is variable m_Name: " << m_Name << std::endl;
        ...
        //m_patchDirection.normalize(); //commented out on 4.nov with thomas
...
//////////////This is working positionVectorFunction
    ...
    virtual Real calculate_angleOfTime (Real maximum_angle, Real minimum_angle, Real time,EMMonodomainSolver<RegionMesh<LinearTetra> > >& solver){};
...
public:
    ...
    void getp1PositionVector(EMSolver<RegionMesh<LinearTetra>, EMMonodomainSolver<RegionMesh<LinearTetra> > >& solver)
    {
    	...
    	for(int j(0); j < p1nCompLocalDof; j++)
    	{
            ...
            std::string path = "/cluster/home/lglaus/LIFE5/lifev-env-rp/lifev-em-install-debug/lifev/em/examples/example_EMHeart/positionvectorfiles/coordinates_" + oss.str() + ".dat";//2020.02.05 lg
       		//std::string path = "/cluster/home/pamstad/LIFE5/lifev-env/lifev-em-build/lifev/em/examples/example_EMHeart/positionvectorfiles/coordinates_" + oss.str() + ".dat";//2020.02.05 lg
    		std::ofstream writer(path.c_str(), std::ios_base::app);
    		if(writer.is_open()==false)
       		{
    			std::cout << "error occured while opening the file" << std::endl;
